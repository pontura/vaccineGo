...
public class ParticleGazeCursor : MonoBehaviour
{
    ...
    void Update()
    {
		...
		var delta = Vector3.zero;//GetComponent<OVRGazePointer>().positionDelta;

        if (particleTrail)
        {
            // Evaluate these curves to decide the emission rate of the two sources of particles.
            emHalf.rateOverTime = new ParticleSystem.MinMaxCurve(halfEmission.Evaluate((delta.magnitude / Time.deltaTime) / maxSpeed) * emissionScale);
            emFull.rateOverTime = new ParticleSystem.MinMaxCurve(fullEmission.Evaluate((delta.magnitude / Time.deltaTime) / maxSpeed) * emissionScale);
